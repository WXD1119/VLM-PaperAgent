...
from paper_agent.graph import GraphQuery, LocalGraphWorkspaceStore, NodeType
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Query a graph workspace effective graph")
    ...
    if not any([args.papers, args.chunks, args.search, args.concept]):
        parser.error("choose one query: --papers, --chunks, --search, or --concept")
    if args.chunks and not args.paper_id:
        parser.error("--chunks requires --paper-id")

    query = GraphQuery(LocalGraphWorkspaceStore(args.workspace).load_effective_graph())
    if args.papers:
